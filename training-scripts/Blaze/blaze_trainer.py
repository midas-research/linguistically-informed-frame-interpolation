...
################################# CREATE DIRS
root_dir = './Viseme_train_outputs_fcn3d_stn_plain_roi_blaze_cnn'
now = str(datetime.now()) + '_scratch'
if test_mode:
    now += '_test' 
...
def prepare_batch(paths, detector):
    ...
    while count < batch_size:
        vid_path = random.choice(train_paths)
...
#     print('Avg Test MSE (int 8 ) Loss', np.average(batch_frame_loss_mse_int8))
    ...
    return np.average(batch_frame_loss_l1), np.average(batch_roi_loss_l1), np.average(batch_total_loss)
...
train_step(model,  optimizerG, train_paths, val_paths, mouth_detector, summary_writer)
