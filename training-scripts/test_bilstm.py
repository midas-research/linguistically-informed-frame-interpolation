...
#         self.conv_lstm3 = ConvLSTM(self.f_maps[1], self.f_maps[2], (3,3), 1, 1, batch_first=True, bias=True, return_all_layers=False, transposed=True, output_padding=1)
...
################################# CREATE DIRS
root_dir = 'train_outputs_lstm_recon_TEST'
now = str(datetime.now()) + '_suffix_40'
if test_mode:
    now += '_test' 
...
model_path = 'train_outputs_lstm_recon/2020-08-30 22:30:00.511302_resumed_scratch/models/1_12000.pt'  # model path
...
def test_step(index, model):
    ...
    for batch,(vin, vout, v_rev, paths) in tqdm(enumerate(test_dataloader)):
        ...
        with torch.no_grad():
            ...
            loss_sl1 = criterion_sl1(out[0], vout)
            loss_l1 = criterion_l1(out[0], vout)
            loss_mse = criterion_mse(out[0], vout)
            
            mse_loss_int8 = criterion_mse(out[0]*255., vout*255.)
            psnr_val = 20 * (torch.log10(255./ torch.sqrt(mse_loss_int8)))
            
            loss_ssim = criterion_ssim(out[0].permute(0,2,1,3,4).mean(1).unsqueeze(1), vout.permute(0,2,1,3,4).mean(1).unsqueeze(1))
            
            test_losses_l1.append(loss_l1.item())
            test_losses_sl1.append(loss_sl1.item())
            test_losses_mse.append(loss_mse.item())
            test_losses_ssim.append(loss_ssim.item())
            test_losses_psnr.append(psnr_val.item())

        summary_writer.add_scalar('test loss L1', loss_l1.item())
        summary_writer.add_scalar('test loss Smooth L1', loss_sl1.item())
        summary_writer.add_scalar('test loss MSE', loss_mse.item())
        summary_writer.add_scalar('test loss SSIM', loss_ssim.item())
        summary_writer.add_scalar('test loss PSRN', psnr_val.item())
        if batch % 1000 == 0 :
            save_sample(vin[0].permute(1,0,2,3), v_rev[0].permute(1,0,2,3), vout[0].permute(1,0,2,3), out[0][0], testdir, 'test_{}_{}_{}'.format(curr_cent, train_index, batch))
    print('TEST Loss L1 : ', np.average(test_losses_l1))
    print('TEST Loss Smooth L1 :', np.average(test_losses_sl1))
    print('TEST Loss MSE : ', np.average(test_losses_mse))
    print('TEST Loss PSNR : ', np.average(test_losses_psnr))
    print('TEST Loss SSIM : ', np.average(test_losses_ssim))
